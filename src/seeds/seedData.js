...
const seedDatabase = async () => {
  try {
    ...
    // Drop collections to ensure old indexes (like 'numero') are completely removed
    const collections = [Table, Menu, Category, Dish, User];
    for (const Model of collections) {
      try {
        await Model.collection.drop();
      } catch (err) {
        // Ignore "ns not found" errors if collection doesn't exist yet
        if (err.code !== 26) console.warn('Could not drop collection', err.message);
      }
    }
    ...
    logger.info(`
      ...
      Test QR codes:
      ${tables.map(t => `Table ${t.table_number}: ${t.qr_code}`).join('\n      ')}
    `);
